...
| Repo name `wedding` | the Pages path; `vite.config.js` sets `base` to `/wedding/` to match |
...
2. **Execute as: Me**. **Who has access: Anyone** — not "Anyone with a Google Account",
   ...
   the board readable without a credential; it is the design, not an oversight.
...
This is the whole access model, and it is two URLs.
...
controls appear from then on. **Treat it like a password**, because it is one wearing a
URL's clothing: anyone you forward it to can edit, and so can anyone who photographs your
address bar.
...
**Paste your edit link**. That is the documented recovery path, not a fallback nobody
thought about.
